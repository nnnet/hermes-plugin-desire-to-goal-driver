...
def _on_pre_llm_call(**kwargs: Any) -> Optional[dict[str, str]]:
    ...
    if _detect_structured_reply(user_msg):
        ...
        exit_block = (
            ...
            "structure-questions. Goal is clear. F1 gate cleared — "
            "all tools (chief_spawn, mc_*, kanban_*, workflow_run, "
            "terminal, execute_code, write_file) are NOW AVAILABLE.\n"
            "\n"
            "**SCOPE ROUTING (CRITICAL — decide before responding):**\n"
            ...
            "team — e.g. «платформа», «стартап», «3-месячный план», "
            "«курс», service with several integrations) → "
            "**YOU MUST EMIT A chief_spawn TOOL CALL THIS TURN.**\n"
            "\n"
            "**LARGE detection (any of these = LARGE):**\n"
            "• Goal text mentions: платформа, стартап, MVP, курс, "
            "сайт с N разделами + оплатой, чатом, проверкой задач, "
            "интеграцией с *.\n"
            "• Context mentions: 3 месяца, 3+ ролей, multiple integrations, "
            "ML, payments + chat + content.\n"
            "• 3+ distinct user-facing features named.\n"
            "\n"
            "**For LARGE — DO NOT respond with text only.** \n"
            "Text «Передаю команде» without a chief_spawn tool call = "
            "BROKEN HANDOFF. The user waits and nothing happens.\n"
            "**CORRECT pattern for LARGE:** ONE tool call to chief_spawn "
            "with arguments = clarified goal as the goal field. Acknow-"
            "ledgement text (if any) goes AFTER the tool result, not "
            "before the call.\n"
            "\n"
            "**For SMALL:** self-execute — direct tool calls without "
